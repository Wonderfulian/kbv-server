/**
 * x402 testnet E2E (PHASE2 stage 2 verification):
 ...
 *   4. watch the receiving address's USDC balance rise on Base Sepolia
 *
...
const USDC = '0x036CbD53842c5426634e7929541eC2318f3dCF7e'; // official Base Sepolia USDC
...
if (payerBefore === 0n) throw new Error('payer holds no test USDC — run the faucet step first');
...
const rpcOptions = { rpcUrl: 'https://sepolia.base.org' };
...
const okUpto = await paidCall('upto batch', `${BASE}/v1/business/batch`, {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ business_numbers: ['124-81-00998', '1234567890'] }),
});
console.log('RESULT | exact:', okExact ? 'PAID' : 'FAILED', '| upto:', okUpto ? 'PAID' : 'FAILED');
