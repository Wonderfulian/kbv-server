...
- [ ] `npm test` 통과 (정규화, 상태 매핑, 캐시, mock API)
- [ ] 로컬에서 MCP Inspector로 두 툴 호출 성공
- [ ] Cloud Run 배포 후 실제 사업자번호로 조회 성공
- [ ] Claude.ai 커스텀 커넥터로 등록해 대화에서 조회 성공
- [ ] 에러 케이스 확인: 잘못된 번호, 미등록 번호, 원천 API 다운(mock)
